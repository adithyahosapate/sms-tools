...
def computeEngEnv(inputFile, window, M, N, H):
	...
	engEnv=np.ones([numFrames,2])

	mX_linear=np.power(10,mX/20.0)

	# print (mX_linear)
	bins =np.arange(0,N)*fs/N
	low_freq_bins=np.where((0<bins)*(bins<3000))
	high_freq_bins=np.where((3000<bins)*(bins<10000))

	for i,frame in enumerate(mX_linear):
		engEnv[i,0]=np.sum(np.square(frame[low_freq_bins]))
		engEnv[i,1]=np.sum(np.square(frame[high_freq_bins]))
	return 10*np.log10(engEnv+eps)
