...
def extractMainLobe(window, M):
	...
	X=fftshift(20*np.log10(abs(fft(w,N))+1e-16))
	mid_point=np.argmax(X)
	print(mid_point)
	print(X)

	for i in range(len(X)):
		if X[mid_point-i]>X[mid_point-i-1]:
			continue
		else:
			min_point=mid_point-i
			break
	...
	for i in range(len(X)):
		if X[mid_point+i]>X[mid_point+i+1]:
			...
			max_point=mid_point+i
			break
	print(min_point)
	print(max_point)
	plt.plot(X)
	plt.show()
	return X[min_point:max_point+1]
